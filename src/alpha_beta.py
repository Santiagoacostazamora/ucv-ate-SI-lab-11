...
Alpha-Beta returns the same decision as Minimax, but avoids exploring branches
that cannot improve the final decision.
"""
...
from src.minimax import _validate_internal_node, is_leaf


@dataclass(frozen=True)
class AlphaBetaResult:
    """Store the value, evaluated leaves and number of pruned branches."""
...
def alpha_beta(
    ...
    """
    Execute Alpha-Beta pruning over a game tree.
    ...
    if maximizing_player:
        ...
        for index, child in enumerate(children):
            ...
            if beta <= alpha:
                branches_pruned += len(children) - index - 1
                break
